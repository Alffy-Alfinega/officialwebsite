...
function buildBranding(scene: Scene): void {
  ...
  scene.registerBeforeRender(() => {
    t += 0.009
    spheres.forEach(({ mesh, baseY }, i) => {
      mesh.rotation.y += 0.007
      mesh.position.y = baseY + Math.sin(t * 0.6 + i * 1.3) * 0.2
    })
    dots.forEach((d, i) => {
...
function buildCybersecurity(scene: Scene): void {
  ...
  scene.registerBeforeRender(() => {
    t += 0.01
    shield.rotation.x += 0.004; shield.rotation.y += 0.007
...
function buildPricing(scene: Scene): void {
  ...
  scene.registerBeforeRender(() => {
    t += 0.01
    gems.forEach(({ mesh, y }, i) => {
      mesh.rotation.x += 0.007 + i * 0.003
      mesh.rotation.y += 0.012 + i * 0.005
      mesh.position.y  = y + Math.sin(t * 0.8 + i * 1.4) * 0.2
    })
  })
}
