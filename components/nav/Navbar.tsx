...
export default function Navbar() {
  ...
  const openDrop  = (label: string) => { clearTimeout(dropdownTimer.current); setActiveDropdown(label) }
  const closeDrop = () => { dropdownTimer.current = setTimeout(() => setActiveDropdown(null), 120) }
  ...
  return (
    <>
      <nav
        ...
        className={cn('fixed top-0 left-0 right-0 z-50 transition-all duration-300', scrolled ? 'backdrop-blur-md border-b' : 'bg-transparent')}
      ...
      >
        <div className="max-w-[1440px] mx-auto px-6 md:px-10 flex items-center justify-between h-[68px]">
          ...
          <Link href="/" className="flex items-center gap-2 group shrink-0" aria-label="Alffy (Alfinega) — Home">
            <Image src="/logo-nav.png" alt="Alffy logo" width={36} height={36} className="transition-all duration-300 group-hover:opacity-85" priority />
