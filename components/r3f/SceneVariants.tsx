...
export function AfricaGlobeScene(){
  ...
  const $=useRef(0)
  useFrame((_,d)=>{
    $.current+=d;if(!g.current)return;const t=$.current
...
export function HelixScene(){
  ...
  const $=useRef(0)
  useFrame((_,d)=>{$.current+=d;if(!g.current)return;const t=$.current;g.current.rotation.y=t*0.18;g.current.position.y=Math.sin(t*0.3)*0.1})
  return(
    <group ref={g}>
...
export function NetworkScene(){
  ...
  const refs=useMemo(()=>nodes.map(()=>createRef<THREE.Mesh>()),[nodes])
  const $=useRef(0)
  useFrame((_,d)=>{
    $.current+=d;if(!g.current)return;const t=$.current
...
export function OrbitalScene(){
  ...
  const meshRefs=useMemo(()=>orbs.map(()=>createRef<THREE.Mesh>()),[orbs])
...
export function PortfolioScene(){
  ...
  const frameRefs=useMemo(()=>frames.map(()=>createRef<THREE.Group>()),[frames])
  const $=useRef(0)
  useFrame((_,d)=>{
    $.current+=d;if(!g.current)return;const t=$.current
...
export function CrystalScene(){
  ...
  const refs=useMemo(()=>crystals.map(()=>createRef<THREE.Mesh>()),[crystals])
  const $=useRef(0)
  useFrame((_,d)=>{
    $.current+=d;if(!g.current)return;const t=$.current
...
export function BlogScene(){
  ...
  const refs=useMemo(()=>docs.map(()=>createRef<THREE.Group>()),[docs])
  const $=useRef(0)
  useFrame((_,d)=>{
    $.current+=d;if(!g.current)return;const t=$.current
...
export function RadarScene(){
  const g=useRef<THREE.Group>(null)
  const ringRefs=useMemo(()=>Array.from({length:5},()=>createRef<THREE.Mesh>()),[])
...
export function SignalScene(){
  ...
  const refs=useMemo(()=>waveGeos.map(()=>createRef<THREE.Mesh>()),[waveGeos])
  const $=useRef(0)
  useFrame((_,d)=>{$.current+=d;if(!g.current)return;const t=$.current;g.current.rotation.y=Math.sin(t*0.2)*0.5;g.current.rotation.x=t*0.06;waveGeos.forEach((_,i)=>{const phase=(t*0.4+i*0.3)%1;if(refs[i].current){refs[i].current.scale.z=0.8+Math.sin(t+i)*0.5}})})
  return(
...
export function FilmReelScene(){
  ...
  const $=useRef(0)
  useFrame((_,d)=>{$.current+=d;if(!g.current)return;const t=$.current;g.current.rotation.z=t*0.35;g.current.rotation.y=Math.sin(t*0.2)*0.5;g.current.position.y=Math.sin(t*0.3)*0.15})
  return(
    <group ref={g}>
...
export function SpiralScene(){
  ...
  const $=useRef(0)
  useFrame((_,d)=>{$.current+=d;if(!g.current)return;const t=$.current;g.current.rotation.y=t*0.15;g.current.position.y=Math.sin(t*0.25)*0.2})
  return(
    <group ref={g}>
...
export function GridBoxScene(){
  ...
  const $=useRef(0)
  useFrame((_,d)=>{$.current+=d;if(!g.current)return;const t=$.current;g.current.rotation.y=t*0.12;g.current.rotation.x=t*0.07;g.current.position.y=Math.sin(t*0.3)*0.1})
  return(
...
export function PrismScene(){
  const g=useRef<THREE.Group>(null)
  const $=useRef(0)
  useFrame((_,d)=>{$.current+=d;if(!g.current)return;const t=$.current;g.current.rotation.y=t*0.18;g.current.rotation.z=t*0.06;g.current.position.y=Math.sin(t*0.3)*0.15})
  return(
    <group ref={g}>
...
export function DataStreamScene(){
  ...
  const colRefs=useMemo(()=>columns.map(()=>createRef<THREE.Points>()),[columns])
  const $=useRef(0)
  useFrame((_,d)=>{
    $.current+=d;if(!g.current)return;const t=$.current
...
export function LensScene(){
  ...
  const $=useRef(0)
  useFrame((_,d)=>{$.current+=d;if(!g.current)return;const t=$.current;g.current.rotation.z=t*0.2;g.current.rotation.y=Math.sin(t*0.15)*0.5;g.current.position.y=Math.sin(t*0.3)*0.12})
  return(
    <group ref={g}>
